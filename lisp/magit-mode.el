...
;;     (("rev-parse" "--verify" "--abbrev-ref" "feature/prefetch2@{upstream}") . string)
;;     (("rev-parse" "--verify" "--abbrev-ref" "fork/feature/prefetch2^{commit}") . string)
;;     (("log" "--no-walk" "--format=%s" "origin/main^{commit}" "--") . string)
;;     (("log" "--no-walk" "--format=%s" "fork/main^{commit}" "--") . string)
;;     (("log" "--no-walk" "--format=%s" "fork/feature/prefetch2^{commit}" "--") . string)
;;     (("describe" "--long" "--tags") . string)
;;     (("describe" "--contains" "HEAD") . string)
;;     (("rev-parse" "--git-dir") . string)
;;     (("rev-parse" "--is-bare-repository") . string)
;;     (("rev-parse" "--short" "HEAD") . string)
;;     (("rev-parse" "--short" "HEAD~") . string)
;;     (("symbolic-ref" "refs/remotes/origin/HEAD") . string)
;;     (("symbolic-ref" "refs/remotes/fork/HEAD") . string)
;;     (("describe" "--contains" "HEAD") . string)
;;     (("rev-parse" "--verify" "HEAD") . string-p)
;;     (("rev-parse" "--verify" "refs/stash") . string-p)
;;     (("rev-parse" "--verify" "HEAD~10") . string-p)
;;     (("rev-parse" "--verify" "refs/tags/feature/prefetch2") . string-p)
;;     (("rev-parse" "--verify" "refs/tags/fork/feature/prefetch2") . string-p)
;;     (("rev-parse" "--verify" "refs/tags/fork/main") . string-p)
;;     (("rev-parse" "--verify" "fork/feature/prefetch2") . string-p)))

(setq magit--prefill-commands-old
  '(;; (("rev-parse" "--show-toplevel") . string)
    ;; (("rev-parse" "--show-cdup") . string)
    ("symbolic-ref" "--short" "HEAD")
    ;; (("log" "--no-walk" "--format=%h" "%s" "HEAD^{commit}" "--"))
    ("rev-parse" "--verify" "--abbrev-ref" "main@{upstream}")
    ("rev-parse" "--verify" "--abbrev-ref" "feature/prefetch2@{upstream}")
    ("rev-parse" "--verify" "--abbrev-ref" "fork/feature/prefetch2^{commit}")
    ("log" "--no-walk" "--format=%s" "origin/main^{commit}" "--")
    ("log" "--no-walk" "--format=%s" "fork/main^{commit}" "--")
    ("log" "--no-walk" "--format=%s" "fork/feature/prefetch2^{commit}" "--")
    ("describe" "--long" "--tags")
    ("describe" "--contains" "HEAD")
    ("rev-parse" "--git-dir")
    ("rev-parse" "--is-bare-repository")
    ...
    ("rev-parse" "--short" "HEAD~")
    ("symbolic-ref" "refs/remotes/origin/HEAD")
    ("symbolic-ref" "refs/remotes/fork/HEAD")
    ("describe" "--contains" "HEAD")
    ("rev-parse" "--verify" "HEAD")
    ("rev-parse" "--verify" "refs/stash")
    ("rev-parse" "--verify" "HEAD~10")
    ("rev-parse" "--verify" "refs/tags/feature/prefetch2")
    ("rev-parse" "--verify" "refs/tags/fork/feature/prefetch2")
...
(defun magit--prefill-sentinel (proc key)
  ;; (message "sentinel: %s %s %s" proc key (process-status proc))
